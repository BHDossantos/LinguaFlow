...
| 12 | STEM + coding lab | ✅/◐ | **Shipped:** a `code` lesson kind + standalone /playground — write JavaScript or Python (Pyodide), auto-graded against hidden tests in a sandboxed browser worker; Python lessons can load numpy/pandas via `packages`. Three hands-on coding courses seeded (JS foundations, Python foundations, **data analysis with pandas**). **Next:** interactive graphs/plots. |
