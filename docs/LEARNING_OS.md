...
| 12 | STEM + coding lab | ✅/◐ | **Shipped:** a `code` lesson kind + standalone /playground — write JavaScript, Python (Pyodide, incl. numpy/pandas via `packages`), or **SQL (sql.js / SQLite)**, all auto-graded in a sandboxed browser worker. Four hands-on coding courses seeded (JS foundations, Python foundations, data analysis with pandas, **practical SQL**). **Next:** interactive graphs/plots. |
