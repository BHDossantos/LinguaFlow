...
| 12 | STEM + coding lab | ✅/◐ | **Shipped:** a `code` lesson kind + standalone /playground — write JavaScript or Python (Pyodide), auto-graded against hidden tests in a sandboxed browser worker. Two hands-on coding courses seeded. **Next:** data-science lessons (pandas/numpy) + interactive graphs. |
